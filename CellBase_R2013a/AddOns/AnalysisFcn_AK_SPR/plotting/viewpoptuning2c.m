function [NORMstimrateC, NORMstimrateE] = viewpoptuning2c(cellids,varargin) 
...
[stimC, FstimC, FstimC_se] = tuning_curve(EpochRate(trialsALL),TE.OdorRatio(trialsALL));
[stimE, FstimE, FstimE_se] = tuning_curve(TE.Correct(trialsALL),TE.OdorRatio(trialsALL));
...
%StandardRatios = [0 32 44 56 68 100];
...
if length(NORMfactor) == 2
    ...
    NORMstimrateE(iC,stimE) = [FstimE(1:Lc)/NORMfactor(1) FstimE(Lc+1:end)];
else
    NORMstimrateC(iC,stimC) = FstimC/NORMfactor;
    NORMstimrateE(iC,stimE) = FstimE;
end

end %ismember StimRate

accubins = [0.5:0.1:0.9];
NORMaccuF(iC,:) = normalize(accubins', NORMstimrateC(iC,:), NORMstimrateE(iC,:),'linear');
...
p1=plot(meanNORMstimrateE(pE),meanNORMstimrateC(pC),'b');
p2=plot(meanNORMstimrateE(pE),meanNORMstimrateC(pC),'bo');
set(p2,'MarkerFaceColor','b');

all=[meanNORMstimrateC meanNORMstimrateE];
axis([0.5 1 min(all)*0.9 max(all)*1.1]);
set([p1],'LineWidth',3);
...
% clf;
% hold on;
% plot(accubins,nanmean(NORMaccuF'));
% 
