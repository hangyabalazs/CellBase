function TE = solo2trialevents4_auditory_gonogo_Hyun(filepath,ifsave)
...
% Correct side
lefttrials = find(saved.auditory_gonogo_Hyun_TrialTypeList(1:ntrials)=='l');
righttrials = find(saved.auditory_gonogo_Hyun_TrialTypeList(1:ntrials)=='r');
...
TE.StimulusID = saved.auditory_gonogo_Hyun_TrialTypes(1:ntrials);
TE.StimulusID = [TE.StimulusID(2:end) NaN];  % remove first trial

% Stimulus duration / sound intensity on every trial
TE.StimulusDuration = saved.auditory_gonogo_Hyun_StimDurList(1:ntrials);
TE.StimulusDuration = [TE.StimulusDuration(2:end) NaN];  % remove first trial
TE.SoundIntensity = saved.auditory_gonogo_Hyun_StimDurList(1:ntrials);   % it is actually sound intensity; however, I keep the redundant StimulusDuration field to avoid errors in other programs - BH
...
TE.LightStimulation = saved.auditory_gonogo_Hyun_LightStimList(1:ntrials);
TE.LightStimulation = [TE.LightStimulation(2:end) NaN];  % remove first trial
TE.LightStimulation2 = saved.auditory_gonogo_Hyun_LightStimulation2(1:ntrials);
...
%     TE.ReactionTime(countr) = diff(saved_history.ProtocolsSection_parsed_events{iT}.states.beginresponse);
    ...
    TE.ITIDistribution(countr) = saved.auditory_gonogo_Hyun_ITIs(iT);
    ...
    % Stimulus
    TE.SignalModality{countr} = saved.auditory_gonogo_Hyun_SignalModality;
...
TE.TrialsPerBlock(:) = saved_history.auditory_gonogo_Hyun_TrialsPerBlock{1};
TE.TrialsPerBlock = [TE.TrialsPerBlock(2:end) NaN];  % remove first trial

blocklength = saved_history.auditory_gonogo_Hyun_TrialsPerBlock{1}*length(unique(TE.StimulusDuration))*2;
