function viewcell2d_speed(cellid,varargin)
    ...
    switch g.LFPType
        ...
        case 'speed'
            ...
            [lfppsth, lfppsth_sd, lfpspsth, lfpspsth_se] = ...
              ...
              [maxspeed ind_MaxSpeed]=max(lfppsth);
          %length(lfppsth)-ind_MaxSpeed=lfppsth(
...
for i=alltrials
stimes{i}  = SP.event_stimes{trigger_pos}{i}-time(ind_MaxSpeed);
windows{i} = SP.event_windows{trigger_pos}(:,i)-time(ind_MaxSpeed);
end
...
hold on
line([0 0], [-5 max(spsth)+max(spsth)/4]);
line([-time(ind_MaxSpeed) -time(ind_MaxSpeed)], [-5 max(spsth)+max(spsth)/4],'color','r');
