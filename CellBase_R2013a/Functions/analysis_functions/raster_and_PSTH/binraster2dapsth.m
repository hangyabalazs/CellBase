function [psth, spsth, spsth_se] = binraster2dapsth(binraster,dt,sigma,COMP,valid_trials)
%BINRASTER2DAPSTH    'Doubly adaptive' PSTH from binned spike raster.
%   [PSTH, SPSTH, SPSTH_SE] = BINRASTER2DAPSTH(BINRASTER,DT,SIGMA,COMP,VALID_TRIALS)
%   calculates 'doubly adaptive' PSTH (see DAPSTH), smoothed PSTH (SPSTH)
%   and standard error of smoothed PSTH (SPSTH_SE) from BINRASTER at time
...
%
%   See also DAPSTH, SMOOTHED_PSTH, STIMES2BINRASTER and VIEWCELL2B.
...
for iCOND = 1:length(positions)
    ...
    if ~isempty(positions{iCOND})  
        [psth(iCOND,:) psth_sd(iCOND,:)] = dapsth(binraster(positions{iCOND},:),dt);  % adaptive PSTH
