function quickanalysis_pavlovian(animalNO,sessionID,sessionspec,protocoltag)
...
if isbeh
    if isempty(protocoltag)
        TE = solo2trialevents_auditory_pavlovian([fullpth 'data_@auditory_pavlovian_balazs_' animalID2 '_' sessionID '.mat']);
    else
        evalstr = ['TE = solo2trialevents4_pavlovian_Sanchari_' protocoltag '([fullpth ''data_@auditory_gonogo_' protocoltag '_balazs_'' animalID2 ''_'' sessionID ''.mat'']);'];
        eval(evalstr)
    end
    if isrec
...
% Behavior
if isbeh
    psychplot_pavlovian({animalID sessionID})
end