function TE = solo2trialevents4_auditory_gonogo_waterpuff(filepath,ifsave)
%SOLO2TRIALEVENTS4_AUDITORY_GONOGO_WATERPUFF   Create trial events structure.
%   SOLO2TRIALEVENTS4_AUDITORY_GONOGO_WATERPUFF(SESSPATH) creates trial events 
...
%
%   SOLO2TRIALEVENTS4_AUDITORY_GONOGO_WATERPUFF(SESSPATH,IFSAVE) uses an
%   addidional logical input argument do determine whether to save the
%   results.
%
...
% Correct side
lefttrials = find(saved.auditory_gonogo_waterpuff_TrialTypeList(1:ntrials)=='l');
righttrials = find(saved.auditory_gonogo_waterpuff_TrialTypeList(1:ntrials)=='r');
...
TE.StimulusID = saved.auditory_gonogo_waterpuff_TrialTypes(1:ntrials);
TE.StimulusID = [TE.StimulusID(2:end) NaN];  % remove first trial

% Stimulus duration / sound intensity on every trial
TE.StimulusDuration = saved.auditory_gonogo_waterpuff_StimDurList(1:ntrials);
TE.StimulusDuration = [TE.StimulusDuration(2:end) NaN];  % remove first trial
TE.SoundIntensity = saved.auditory_gonogo_waterpuff_StimDurList(1:ntrials);   % it is actually sound intensity; however, I keep the redundant StimulusDuration field to avoid errors in other programs - BH
...
TE.LightStimulation = saved.auditory_gonogo_waterpuff_LightStimList(1:ntrials);
TE.LightStimulation = [TE.LightStimulation(2:end) NaN];  % remove first trial
if isfield(saved,'auditory_gonogo_waterpuff_LightStimulation2')
    TE.LightStimulation2 = saved.auditory_gonogo_waterpuff_LightStimulation2(1:ntrials);
...
%     TE.ReactionTime(countr) = diff(saved_history.ProtocolsSection_parsed_events{iT}.states.beginresponse);
    ...
    TE.ITIDistribution(countr) = saved.auditory_gonogo_waterpuff_ITIs(iT);
    ...
    % Stimulus
    TE.SignalModality{countr} = saved.auditory_gonogo_waterpuff_SignalModality;
...
TE.TrialsPerBlock(:) = saved_history.auditory_gonogo_waterpuff_TrialsPerBlock{1};
TE.TrialsPerBlock = [TE.TrialsPerBlock(2:end) NaN];  % remove first trial

blocklength = saved_history.auditory_gonogo_waterpuff_TrialsPerBlock{1}*length(unique(TE.StimulusDuration))*2;
