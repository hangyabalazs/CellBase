function viewcells(cellids,varargin)
% viewcells(cellid,varargin)
% Plots for a number of cellids, a raster psth for the first cellid ...
% and psths for the others. The cells should be from the same session inputs:
% cellid: (required)
% EV: structure of events that include TriggerEvent (required)
% TriggerEvent 'TargetSoundOn' OR TriggerEvent 'PulseOn' (required)
% filetype: 'stim';'event'
% Partitions: default?
...
if length(cellids)==1,
    switch g.mates2plot
        case 'tetrode'
            [NumCells,plotcellids]=tetrodepairs(cellids);
            cellids=[cellids setdiff(plotcellids,cellids)];
        case 'session'
            [r,s,t,u]=cellid2tags(cellids);
            sess_cellids=findcell('rat',r,'session',s);
            cellids=[cellids setdiff(sess_cellids,cellids)];
        ...
        case 'session_select'
            ...
            cellids=[cellids setdiff(sess_cellids,cellids)];
            NumCells=4;
%             1 5 6 9 21 24(11:13 are all light activated)
            cellids=cellids([1 5 6 9]);
    end
else
    NumCells=length(cellids);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
try
    fhandles=preparefigure(NumCells,'1rnp',g.FigureNum);
catch
    fhandles=preparefigure(NumCells,'nrnp',g.FigureNum);
end
figco=1;
for iC=1:length(cellids),
    cellid=cellids(iC);
    g.FigureNum=fhandles(figco);
    ...
    valid_trials=valid_trials(TE.NextPulseOn(valid_trials)-TE.PulseOn(valid_trials)>0.05);
    ...
    g.FigureNum=fhandles(figco);
    if iC==1,
        fhandle0 = plot_raster2a(stimes,time,valid_trials,COMPTRIALS,mylabels,EventTimes,window_margin,ev_windows,sort_var,g,'PSTH','off','Colors',{mycolors},'Colors2',{mycolors2},'NumTrials2Plot',g.Num2Plot);
        figco=figco+1;
    end
    ...
    g.FigureNum=fhandles(figco);
    plot_timecourse(time,spsth,spsth_se,g,'FigureNum',g.FigureNum,'Colors',{mycolors},'LineStyle',{mylinestyle},'Legend',{mylabels},'XLabel',XLabel,'YLabel',YLabel);
    set(gca,'TickDir','out')
    figco=figco+1;
