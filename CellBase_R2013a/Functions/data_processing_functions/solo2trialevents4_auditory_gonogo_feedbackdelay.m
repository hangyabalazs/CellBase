function TE = solo2trialevents4_auditory_gonogo_feedbackdelay(filepath,ifsave)
...
% Correct side
lefttrials = find(saved.auditory_gonogo_feedbackdelay_TrialTypeList(1:ntrials)=='l');
righttrials = find(saved.auditory_gonogo_feedbackdelay_TrialTypeList(1:ntrials)=='r');
...
TE.StimulusID = saved.auditory_gonogo_feedbackdelay_TrialTypes(1:ntrials);
TE.StimulusID = [TE.StimulusID(2:end) NaN];  % remove first trial

% Stimulus duration / sound intensity on every trial
TE.StimulusDuration = saved.auditory_gonogo_feedbackdelay_StimDurList(1:ntrials);
TE.StimulusDuration = [TE.StimulusDuration(2:end) NaN];  % remove first trial
TE.SoundIntensity = saved.auditory_gonogo_feedbackdelay_StimDurList(1:ntrials);   % it is actually sound intensity; however, I keep the redundant StimulusDuration field to avoid errors in other programs - BH
...
TE.LightStimulation = saved.auditory_gonogo_feedbackdelay_LightStimList(1:ntrials);
TE.LightStimulation = [TE.LightStimulation(2:end) NaN];  % remove first trial
TE.LightStimulation2 = saved.auditory_gonogo_feedbackdelay_LightStimulation2(1:ntrials);
...
%     TE.ReactionTime(countr) = diff(saved_history.ProtocolsSection_parsed_events{iT}.states.beginresponse);
    ...
    TE.ITIDistribution(countr) = saved.auditory_gonogo_feedbackdelay_ITIs(iT);
    ...
    % Stimulus
    TE.SignalModality{countr} = saved.auditory_gonogo_feedbackdelay_SignalModality;
...
TE.SoundDuration(:) = saved.auditory_gonogo_feedbackdelay_SoundDuration;
TE.SoundDuration = [TE.SoundDuration(2:end) NaN];  % remove first trial

% Sound Frequency
TE.SoundFrequency = nan(1,ntrials);
TE.SoundFrequency(:) = saved.auditory_gonogo_feedbackdelay_SoundFrequency;
TE.SoundFrequency = [TE.SoundFrequency(2:end) NaN];  % remove first trial

% Response
TE.ResponseDelay = nan(1,ntrials);
TE.ResponseDelay(:) = saved.auditory_gonogo_feedbackdelay_Delay2Resp;
...
TE.TrialsPerBlock(:) = saved_history.auditory_gonogo_feedbackdelay_TrialsPerBlock{1};
TE.TrialsPerBlock = [TE.TrialsPerBlock(2:end) NaN];  % remove first trial

blocklength = saved_history.auditory_gonogo_feedbackdelay_TrialsPerBlock{1}*length(unique(TE.StimulusDuration))*2;
