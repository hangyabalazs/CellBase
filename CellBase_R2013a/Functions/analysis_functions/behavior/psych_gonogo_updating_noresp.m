...
for iM = 1:NumMice   % loop through mice
    ...
    for iS = 1:NumSessions   % loop through sessions
        ...
        [GPcp NGPcp GORTp NGRTp] = psychplot_gonogo({animalID sessionID},'display',false,...
        ...
        if any(isnan(GPcp)) || any(isnan(GPcm)) || ...
            any(isnan(NGPcp)) || any(isnan(NGPcm)) % some conditions have no trials: not enough data
            continue
        end
        SI = unique(TE.SoundIntensity(1:end-1));   % different stimulus intesities
        if ~isequal(length(SI),length(GPcp),length(GPcm)) % some conditions have no trials: not enough data
            continue
        end
        GoPerformance = [GoPerformance GPcp-GPcm]; %#ok<AGROW>
        NoGoPerformance =[NoGoPerformance NGPcp-NGPcm]; %#ok<AGROW>
        GoReactionTime =[GoReactionTime GORTp-GORTcm]; %#ok<AGROW>
        NoGoReactionTime = [NoGoReactionTime NGRTp-NGRTcm]; %#ok<AGROW>
