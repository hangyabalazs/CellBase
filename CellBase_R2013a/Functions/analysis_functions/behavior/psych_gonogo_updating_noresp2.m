...
for iM = 1:NumMice   % loop through mice
    ...
    for iS = 1:NumSessions   % loop through sessions
        ...
        cr_inx = intersect(2:NumTrials-2,find(TE.CorrectRejection==1));
        miss_inx = intersect(2:NumTrials-2,find(TE.Miss==1));
        cr_inxm = unique([cr_inx-3 cr_inx-4 cr_inx-5]);
        cr_inxm(cr_inxm<1) = [];
        miss_inxm = unique([miss_inx-3 miss_inx-4 miss_inx-5]);
        miss_inxm(miss_inxm<1) = [];

        [GPcp NGPcp GORTcp NGRTcp] = psychplot_gonogo({animalID sessionID},'display',false,...
            'valid_trials',cr_inx+1);
        [GPcm NGPcm GORTcm NGRTcm] = psychplot_gonogo({animalID sessionID},'display',false,...
            'valid_trials',cr_inxm);
        [GPmp NGPmp GORTmp NGRTmp] = psychplot_gonogo({animalID sessionID},'display',false,...
            'valid_trials',miss_inx+1);
        [GPmm NGPmm GORTmm NGRTmm] = psychplot_gonogo({animalID sessionID},'display',false,...
            'valid_trials',miss_inxm);
        if any(isnan(GPmp)) || any(isnan(GPmm)) || ...
            any(isnan(NGPmp)) || any(isnan(NGPmm)) % some conditions have no trials: not enough data
            continue
        end
        SI = unique(TE.SoundIntensity(1:end-1));   % different stimulus intesities
        if ~isequal(length(SI),length(GPmp),length(GPmm)) % some conditions have no trials: not enough data
            continue
        end
        GoPerformance = [GoPerformance GPmp-GPmm]; %#ok<AGROW>
        NoGoPerformance =[NoGoPerformance NGPmp-NGPmm]; %#ok<AGROW>
        GoReactionTime =[GoReactionTime GORTmp-GORTmm]; %#ok<AGROW>
        NoGoReactionTime = [NoGoReactionTime NGRTmp-NGRTmm]; %#ok<AGROW>
