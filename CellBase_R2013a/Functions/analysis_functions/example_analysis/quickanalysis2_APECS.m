function quickanalysis2_APECS(animalID,sessionID,sessionspec,protocoltag)
...
%   system.
%
%   QUICKANALYSIS2(ANIMALID,SESSIONID,SESSIONSPEC) performs the analysis
...
%
%   QUICKANALYSIS2(ANIMALID,SESSIONID,SESSIONSPEC,PROTOCOLTAG) accepts a
...
% Directories
global DATAPATH
resdir = [DATAPATH 'APECS\trajectories\' animalID '\'];
if ~isdir(resdir)
    mkdir(resdir)
end
% resdir2 = [DATAPATH 'APECS\_behavior\' animalID '\'];
% if ~isdir(resdir2)
%     mkdir(resdir2)
% end
...
load([fullpth 'EVENTS.mat'])   % load Neuralynx events

% Zone entry and exit times
ZoneInTimes = Events_TimeStamps(Events_EventIDs==6&Events_Nttls==1);
ZoneOutTimes = Events_TimeStamps(Events_EventIDs==6&Events_Nttls==0);
ZoneInX = interp1q(TimeStamps',ExtractedX',ZoneInTimes')';
ZoneInY = interp1q(TimeStamps',ExtractedY',ZoneInTimes')';
ZoneOutX = interp1q(TimeStamps',ExtractedX',ZoneOutTimes')';
ZoneOutY = interp1q(TimeStamps',ExtractedY',ZoneOutTimes')';

H = figure;
plot(ExtractedX,ExtractedY,'k.')
hold on
plot(ZoneInX,ZoneInY,'g.','MarkerSize',12)
plot(ZoneOutX,ZoneOutY,'r.','MarkerSize',12)
maximize_figure(H)
% keyboard

% Save
fnm = [resdir sessionID '_trajectory.fig'];   % save
saveas(H,fnm)
axis off
fnm = [resdir sessionID '_trajectory.jpg'];   % save
saveas(H,fnm)
close(H)
