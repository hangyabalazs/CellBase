function TE = solo2trialevents3_auditory_gonogo(filepath,ifsave)
%SOLO2TRIALEVENTS3_AUDITORY_GONOGO   Creates trial events structure (new version of restart ITI).
%   SOLO2TRIALEVENTS3_AUDITORY_GONOGO(SESSPATH) creates trial events 
...
%
%   SOLO2TRIALEVENTS3_AUDITORY_GONOGO(SESSPATH,IFSAVE) uses an addidional
%   logical input argument do determine whether to save the results.
%
%   SOLO2TRIALEVENTS3_AUDITORY_GONOGO is designed for the new type of
%   restart ITI structure (multiple ITI states, independently drawn).
%
...
for itiss = 2:15
    eval(['TE.ITIDistribution' num2str(itiss) ' = nan(1,ntrials);'])
end
...
%     TE.ReactionTime(countr) = diff(saved_history.ProtocolsSection_parsed_events{iT}.states.beginresponse);
    ...
    TE.ITIDistribution2(countr) = saved.auditory_gonogo_ITIs2(iT);
    allitis = [saved_history.ProtocolsSection_parsed_events{iT}.states.iti;...
        saved_history.ProtocolsSection_parsed_events{iT}.states.iti2b];
    for itiss = 3:15
        eval(['TE.ITIDistribution' num2str(itiss) '(countr) = saved.auditory_gonogo_ITIs' num2str(itiss) '(iT);']);
        allitis = [allitis; eval(['saved_history.ProtocolsSection_parsed_events{iT}.states.iti' num2str(itiss) ';'])];
    end
    TE.ITIBegins{countr} = allitis(:,1);
    TE.ITIEnds{countr} = allitis(:,2);
