function TE = solo2trialevents5_auditory_gonogo(filepath,ifsave)
%SOLO2TRIALEVENTS5_AUDITORY_GONOGO   Create trial events structure.
%   SOLO2TRIALEVENTS5_AUDITORY_GONOGO(SESSPATH) creates trial events
...
%
%   SOLO2TRIALEVENTS5_AUDITORY_GONOGO(SESSPATH,IFSAVE) uses an addidional
...
TE.LastITIBegins = nan(1,ntrials);
TE.LastITIEnds = nan(1,ntrials);
...
for iT = 2:ntrials
    ...
    TE.LastITIBegins(countr) = TE.ITIBegins{countr}(end);   % last ITI start
    TE.LastITIEnds(countr) = TE.ITIEnds{countr}(end);   % last ITI ends
...
TE.LastITIBegins = TE.LastITIBegins - TE.TrialStart;
TE.LastITIEnds = TE.LastITIEnds - TE.TrialStart;
